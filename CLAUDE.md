...
- **macOS training launch**: the engine's `train.py` builds its env creator as a `functools.partial` of a module-level function specifically so it pickles under macOS's default `spawn` start method — do NOT "simplify" it back to a closure, and do not force the `fork` start method on darwin. Fork after torch/PyBoy/SDL load is unreliable even with `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES`: workers intermittently die at fork with no traceback or crash report (zombie children) while pufferlib's main process spin-polls at 100% CPU looking like a hung run at SPS 0 — if you see that pattern, check `ps` for `<defunct>` children of the trainer. `train_macos.sh` still sets the env var (harmless) but the spawn fix is what matters. Separately, the engine's `config.yaml` must set `train.device: cpu` (or `mps`) on Mac; `cuda` raises `AssertionError: Torch not compiled with CUDA enabled`.
